...
from coordwatch.utils.treasury import download_link_records, extract_links, filter_links

KEYWORDS = ["borrowing estimates", "financing estimates", "tbac", "minutes", "advisory committee"]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Download Treasury financing-estimate and TBAC artifacts")
    parser.add_argument("--download-files", action="store_true")
    args = parser.parse_args()

    ensure_repo_dirs()
    manifest = load_source_manifest()["treasury"]
    pages = [manifest["refunding_most_recent"], manifest["refunding_archives"], manifest["refunding_process"]]
    all_links = []
    for page in pages:
        try:
    ...
    filtered = filter_links(all_links, KEYWORDS)
    filtered_df = pd.DataFrame(filtered).drop_duplicates()
    out_dir = RAW_DIR / "downloads" / "treasury" / "financing"
    out_dir.mkdir(parents=True, exist_ok=True)
    write_csv(filtered_df, out_dir / "link_index.csv")
    logger.info("Wrote financing link index with %s rows", len(filtered_df))
